...
foreign import ccall unsafe "" c_set_rawOutput :: Ptr RawOutput -> Ptr CChar -> Ptr CChar -> Ptr CChar -> Ptr RawBuffer -> IO ()
foreign import ccall unsafe "" c_set_rawInput :: Ptr RawInput -> Bool
                                                              -> Ptr CChar -> CUInt
                                                              -> Ptr CChar -> Ptr CChar -> Ptr RawBuffer
                                                              -> CUInt
                                                              -> Ptr CChar -> Ptr CChar -> Ptr CChar -> Ptr CChar -> IO ()
...
withIssuance :: Maybe Issuance -> (Ptr CChar -> Ptr CChar -> Ptr CChar -> Ptr CChar -> IO b) -> IO b
withIssuance Nothing k = withArray (replicate 32 0) $ \pZero -> k pZero pZero nullPtr nullPtr
withIssuance (Just (Left newIssue)) k =
  ...
  maybeAmount (newIssuanceAmount newIssue) $ \pAmount ->
  maybeAmount (newIssuanceTokenAmount newIssue) $ \pToken ->
  k pZero pContract pAmount pToken
 where
  maybeAmount (Amount (Explicit 0)) k = k nullPtr
  maybeAmount amt k = BS.useAsCString (encode amt) k
...
withIssuance (Just (Right reIssue)) k =
  ...
  BS.useAsCString (encode (reissuanceAmount reIssue)) $ \pAmount ->
  k pBlinding pEntropy pAmount nullPtr
...
withRawOutputs txos k =
 ...
 where
  ...
  pokeRawOutput txo pRawOutput k =
    BS.useAsCString (encode (txoAsset txo)) $ \pAsset ->
    BS.useAsCString (encode (txoAmount txo)) $ \pAmount ->
    withMaybe (txoNonce txo) $ \pNonce ->
    withRawBuffer (txoScript txo) $ \pScript -> do
      c_set_rawOutput pRawOutput pAsset pAmount pNonce pScript
      k
...
withRawInputs txis k =
 ...
 where
  ...
  pokeRawInput txi pRawInput k =
    ...
    BS.useAsCString (encode . utxoAsset $ sigTxiTxo txi) $ \pAsset ->
    BS.useAsCString (encode . utxoAmount $ sigTxiTxo txi) $ \pValue ->
    withRawBuffer (utxoScript $ sigTxiTxo txi) $ \pScript ->
    withIssuance (sigTxiIssuance txi) $ \pBlindingNonce pAssetEntropy pAmount pInflationKeys -> do 
