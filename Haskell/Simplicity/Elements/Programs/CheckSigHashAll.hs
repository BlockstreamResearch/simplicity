...
-- | Given a public key and a schnorr signature, this Simplicity program asserts that the signature is valid for the public key and the message generated by 'sigHashAll'.
...
checkSigHashAll :: (Assert term, Primitive term, Witness term) => LibSecp256k1.Lib term -- ^ "Simplicity.Programs.LibSecp256k1"
                                                               -> Lib term
                                                               -> Schnorr.XOnlyPubKey -> Schnorr.Sig -> term () ()
checkSigHashAll libsecp256k1 Lib{..} (Schnorr.XOnlyPubKey x) ~(Schnorr.Sig r s) =
   (scribe (toWord256 . toInteger $ x) &&& sigHashAll) &&& (witness (toWord256 . toInteger $ r, toWord256 . toInteger $ s))
   >>> schnorrAssert
 where
  schnorrAssert = LibSecp256k1.schnorrAssert libsecp256k1
